...
paragraph text instead of parsing as fenced div markers.
...
:   Pandoc fenced divs use `:::` (or longer colon runs) for both openers (with
    ...
    text inside a paragraph. Code spans (`` `:::` ``), indented code blocks, and raw
    HTML blocks are not flagged because the colons there are not paragraph text.
    Authentic prose mentions of `:::` (writing about Pandoc syntax) should be
    wrapped in backticks anyway---runs of three or more consecutive colons are
    otherwise extremely rare in natural text.
