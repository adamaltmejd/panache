...
* `--line-width <N>` — Override the maximum line width for wrapping. Takes precedence over `line-width` in panache.toml and the built-in default (80). Must be a positive integer.
* `--wrap <MODE>` — Override how paragraphs are wrapped. Takes precedence over `[format] wrap` in panache.toml. Options: `reflow` (reformat to fit line width, the default), `sentence` (wrap after each sentence), `preserve` (keep existing line breaks).
