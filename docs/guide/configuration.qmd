...
config closer to the file wins over one at the project root. The walk stops at
the nearest `.git` ancestor (project root) so unrelated configs above the
project --- e.g. a stray `panache.toml` in your home directory --- are not
inherited. The same rule applies to both the CLI and the LSP. If no `.git`
ancestor exists (e.g. an untracked scratch file), the walk falls back to today's
...
The `--line-width` flag on `panache format` overrides this setting for a single
invocation.
...
The `--wrap` flag on `panache format` overrides this setting for a single
invocation.
